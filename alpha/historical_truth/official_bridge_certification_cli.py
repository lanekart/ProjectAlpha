...
def official_bridge_evidence_discovery_registry(
    dossiers: Path = typer.Option(..., "--dossiers", exists=True, dir_okay=False),
    output: Path = typer.Option(
        Path("artifacts/htr010b1f_official_bridge_evidence_discovery_2026"),
        "--output",
    ),
) -> None:
    """Build one blank governed discovery row per official-evidence dossier."""

    report = build_discovery_registry(dossiers_path=dossiers)
    paths = export_discovery_registry(report, output)
    print("HTR-010B1F Official Bridge Evidence Discovery Registry")
    print(f"Contract: {report['contract_version']}")
    print(f"Input dossiers: {report['input_dossier_count']:,}")
    print(f"Discovery templates: {report['discovery_template_count']:,}")
    print(f"Populated discoveries: {report['populated_discovery_count']:,}")
    print(f"Implementation defects: {report['implementation_defect_count']:,}")
    print(f"Report SHA256: {report['report_sha256']}")
    print(f"Artifacts written: {len(paths)}")
    print("Evidence downloads: 0")
