...
def decision_superiority_gate_value(
    ...
    outcome_coverage_ledger: Annotated[
        Path, typer.Option("--outcome-coverage-ledger")
    ],
    ...
    positive = [row for row in values if row["conclusion"] == "GATE_ADDS_MEASURABLE_VALUE"]
    negative = [
    ...
    typer.echo(
        ...
        + (str(max(positive, key=lambda row: row["net_gate_value"])["gate_code"]) if positive else "NONE")
    )
    typer.echo(
        "Highest negative gate value: "
        + (str(min(negative, key=lambda row: row["net_gate_value"])["gate_code"]) if negative else "NONE")
    )
